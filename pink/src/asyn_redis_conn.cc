...
AsynRedisConn::~AsynRedisConn() {
  free(rbuf_);
}

ReadStatus AsynRedisConn::ProcessInlineBuffer() {
  int pos, ret;
...
ReadStatus AsynRedisConn::ProcessMultibulkBuffer() {
  int pos = 0;
  if (multibulk_len_ == 0) {
...
ReadStatus AsynRedisConn::ProcessInputBuffer() {
  ReadStatus ret;
...
WriteStatus AsynRedisConn::SendReply() {
  ssize_t nwritten = 0;
...
int AsynRedisConn::FindNextSeparators() {
  if (next_parse_pos_ > last_read_pos_) {
    return -1;
  }
...
int AsynRedisConn::GetNextNum(int pos, long* value) {
  std::string tmp;
