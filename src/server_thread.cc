...
ServerThread::ServerThread(const std::string& bind_ip, int port, int cron_interval = 0) :
  cron_interval_(cron_interval),
  port_(port) {
  ips_.insert(bind_ip);
}

ServerThread::ServerThread(const std::set<std::string>& bind_ips, int port, int cron_interval = 0) :
  cron_interval_(cron_interval),
  port_(port) {
  ips_ = bind_ips;
}
